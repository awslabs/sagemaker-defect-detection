...
find . ! -name "source_dir.tar.gz" -type f -exec rm -r {} + && rm -rf sagemaker_defect_detection
...
rm build/notebooks/*.json >/dev/null 2>&1 # if used in dev
nbstripout build/notebooks/*.ipynb
black-nb build/notebooks/*.ipynb
for nb in build/notebooks/*.ipynb; do
python ./scripts/set_kernelspec.py --notebook $nb --display-name "Python 3 (PyTorch JumpStart)" --kernel "HUB_1P_IMAGE"
done
...
cd build/solution-assistant/src
