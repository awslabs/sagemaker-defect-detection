...
def delete_sagemaker_endpoint_config(endpoint_config_name):
    ...
    try:
        ...
        print("Successfully deleted endpoint configuration " "called '{}'.".format(endpoint_config_name))
    except sagemaker_client.exceptions.ClientError as e:
        if "Could not find endpoint configuration" in str(e):
            print(
                "Could not find endpoint configuration called '{}'. " "Skipping delete.".format(endpoint_config_name)
            )
        else:
            raise e
...
def on_delete(event, __):
    ...
    endpoint_names = [
        "{}-demo-endpoint".format(solution_prefix),  # make sure it is the same as your endpoint name
        "{}-demo-model".format(solution_prefix),
        "{}-finetuned-endpoint".format(solution_prefix),
        "{}-detector-from-scratch-endpoint".format(solution_prefix),
        "{}-classification-endpoint".format(solution_prefix),
    ]
